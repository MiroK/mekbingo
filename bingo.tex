...
\newcommand{\randomwords}{%
  ...
  observations; oscillations; error; measure; absorb; target; distribution; mean; deviation; smooth
}

% NOTE: the algorithm for filling in the card is to take a random work from
% the list taking it out and select again ...
...
\pgfmathsetseed{SEED}  % This is a template file which is seeded by bingo.sh
...
\begin{tikzpicture}[scale=1, every node/.style={scale=1}]
  \foreach \f in {0,...,\fieldcount}
           { \pgfmathtruncatemacro{\x}{mod(\f,\gridsize)}
             ...
             \ifthenelse{\f=\bingo}
                        {\node[] at
                          ({(\x+0.5)*\cellsize},{(\y+0.5)*\cellsize}) {\includegraphics[width=0.08\textwidth, height=0.08\textwidth]{./img/freak_wave.jpg}};
                        }
           ...
           { \pgfmathtruncatemacro{\x}{mod(\f,\gridsize)}
             ...
             \ifthenelse{\f=\bingo}
                        ...
                        { \pgfmathtruncatemacro{\maxvalue}{\numwords-1-\f)}
                          \pgfmathtruncatemacro{\myrandom}{random(\maxvalue)}
                          \pgfmathtruncatemacro{\mynextrandom}{\myrandom+1}
                          \StrBetween[\myrandom, \mynextrandom]{\randomwordss}{;}{;}[\randomword]
                          \StrDel{\randomwordss}{\randomword;}[\randomwordss]
                          \xdef\randomwordss{\randomwordss}
                          \node[rotate=45] at ({(\x+0.5)*\cellsize},{(\y+0.5)*\cellsize}) {\randomword};
                        }
                        \ifthenelse{\x=0}
                                   {\draw[fill=red!10!white]
